...
2. The `uds-release` GitHub App must be installed on this repository with Contents read/write, Issues read/write, Pull requests read, and Metadata read permissions. Its client ID must be stored as the `APP_CLIENT_ID` variable and its private key must be stored as the `APP_PRIVATE_KEY` secret in the `uds-release` GitHub Environment. The environment must be restricted to protected release branches, including `main` and `release/*`.
