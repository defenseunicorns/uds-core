...
3. **Always-enforced PKCE S256 for non-device-flow public clients.** When the flag is on, the admission validator rejects any public client whose `pkce.code.challenge.method` attribute is not exactly `S256` (case-sensitive per RFC 7636). `plain` is rejected because it transmits the challenge equal to the verifier on the wire, so a stolen authorization code can be redeemed directly, which defeats the only mitigation PKCE is meant to provide. Clients that cannot emit `S256` have to be created outside the UDS Operator (Admin API, OpenTofu) as a conscious, out-of-band decision.
