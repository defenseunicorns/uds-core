...
const sendLog = async (
    ...
    try {
        ...
        const response = await fetch(`${lokiWrite.url}/loki/api/v1/push`, {
            method: 'POST',
...
const queryLogs = async (query: string, limit = 1): Promise<any> => {
    try {
        const response = await fetch(`${lokiRead.url}/loki/api/v1/query_range?query=${encodeURIComponent(query)}&limit=${limit}`, {
            method: 'GET',
        });
