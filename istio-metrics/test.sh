...
help() {
  ...
  echo "  --duration <number> Time in seconds to run (default: $VEGETA_DURATION)"
...
NAMESPACES=("nginx" "istio-system" "istio-tenant-gateway")
...
total_duration=$(echo "$VEGETA_DURATION" | sed 's/s//')

# Run Vegeta in the background
echo "🚀 Starting Vegeta load test: $VEGETA_RATE requests/sec for $VEGETA_DURATION on ${NGINX_REPLICAS} pod(s)"
(echo "GET $VEGETA_TARGET/" | vegeta attack -rate="$VEGETA_RATE" -duration="$VEGETA_DURATION" | tee "$VEGETA_OUTPUT" > /dev/null) &
VEGETA_PID=$!

# Shell-native time indicator
elapsed_seconds=0

while [[ -d /proc/$VEGETA_PID ]]; do
  remaining_seconds=$((total_duration - elapsed_seconds))
  printf "\rRemaining time: %d seconds" "$remaining_seconds" # Overwrite the current line
...
# vegeta report in the foreground
echo "📊 Vegeta Report:"
vegeta report "$VEGETA_OUTPUT"
