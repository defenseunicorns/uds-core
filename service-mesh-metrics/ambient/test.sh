...
VEGETA_RATE="50"      # Requests per second
NGINX_REPLICAS=90     # Number of replicas for the test
...
collect_metrics() {
  ...
  for ns in "${NAMESPACES[@]}"; do
    kubectl top pods -n "$ns" --no-headers | awk -v ts="$timestamp" -v ns="$ns" '
    {
...
echo "GET https://$VEGETA_TARGET/" | vegeta attack -rate="$VEGETA_RATE" -duration="$VEGETA_DURATION" | tee "$VEGETA_OUTPUT" | vegeta report
