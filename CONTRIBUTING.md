...
- **Improve Documentation**: Contributions to documentation (e.g. tutorials, configuration reference, even code comments) are valuable. If you find areas of the docs that can be clearer, you can submit changes. Follow the [style rules](docs/style-rules.md) and [voice profile](docs/voice-profile.md) for writing conventions, and the templates in `.ai/docs/` for page structure.
