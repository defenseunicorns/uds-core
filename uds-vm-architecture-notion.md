...
The first iteration focuses on running and integrating VM workloads on top of Kubernetes in UDS environments. It is meant for environments that need some VM workloads on the platform, not for replacing every capability of a large general-purpose hypervisor platform in the first release.
...
At a high level, UDS VM is a capability that sits on top of `uds-core` and packages the runtime, image path, and operator workflows needed to run VM workloads in a UDS environment. The main components and their relationships are shown below.
...
Platform engineer
       ...
       v                             v
+---------------------+   +---------------------------+
| browser UI          |   | VM image build /          |
| management path     |   | packaging workflow        |
+----------+----------+   +-------------+-------------+
           |                            |
           | interacts with             | prepares artifacts for
           v                            v
+-----------------------------------------------------------+
|                          UDS VM                           |
| +---------------------+    uses    +---------------------+ |
| |      KubeVirt       |<---------->|         CDI         | |
| | VM runtime + API    |            | disk import/upload  | |
| +----------+----------+            +---------------------+ |
+------------+----------------------------------------------+
             |                                        |
             | provides capability for                | relies on platform integration from
             v                                        v
+---------------------+                        +-----------------------------------------------------------+
|    VM workloads     |                        |                         uds-core                          |
| VirtualMachine      |                        | Package API | operator + Pepr | namespace integration     |
| DataVolume/Service  |                        | required VM mutation hooks                                |
+----------+----------+                        +-----------------------------------------------------------+
           ^
           |
application teams define VM workloads in app packages
```

This architecture has a few important parts:

- Platform operators interact with UDS VM through the supported browser for first-iteration tasks such as basic VM information, power actions, and console access.
- A separate VM image build and packaging workflow may also exist alongside the runtime path. It prepares or customizes VM images and turns them into deployable artifacts before UDS VM delivers and runs them through the supported image path. (see current [vm-bridge repo](https://github.com/defenseunicorns-labs/virtual-machine-bridge-uds))
- UDS VM is the capability layer on top of `uds-core`. It carries the supported management path, the package-driven common path for VM workloads, supported defaults, and the VM runtime integration.
- `KubeVirt` is the VM runtime and API surface. It owns the Kubernetes-side VM control plane and the resources used to run VMs.
- `CDI` (Containerized Data Importer) handles the disk image path for imports, uploads, and PVC-backed workflows that feed VM disks into `KubeVirt`.
- Application teams still own the VM workloads they deploy, but the supported path should start from a UDS VM package-driven common path rather than from raw upstream objects assembled from scratch.
- `uds-core` provides the narrow platform hooks that let VM workloads fit the normal UDS admission, policy, and namespace model.
...
The first iteration should not introduce a broad UDS-owned replacement API or a new general-purpose VM operator. It should provide a package-driven common path for the VM cases UDS VM supports, while still using native `KubeVirt` and `CDI` resources underneath. That path should ship as supported package templates, manifests, and defaults so teams can start from a supported VM package shape instead of assembling raw upstream objects from scratch. Teams that need something outside that path should still be able to use the runtime's native API directly.
...
**Decision:** The first iteration should provide browser-based management access for basic VM information, power actions, and console access.

Prototype work is already tracked in [RA-249](https://linear.app/defense-unicorns/issue/RA-249/kubevirt-vm-integration). The recommended first path is to reuse and harden an existing OSS KubeVirt management UI rather than build a fully custom UDS VM interface in the first iteration. The current best-fit path to evaluate first is a narrower UI built on an existing surface such as Headlamp plus a KubeVirt plugin, because that is closer to the required first-iteration operator workflow than a broader standalone VM management product. If no OSS option can meet the UDS security and support bar, the fallback path is a product-owned wrapper or narrower custom UI for those same workflows. Browser-based remote access is part of this requirement, but session-brokering tools such as Guacamole solve only the connect portion of the problem and are not a complete substitute for a VM management UI. The current `remote-agent` work is best treated as a source of implementation ideas and reusable UI patterns, not as the architecture answer by itself.
...
- **Operational workflows:** The first iteration should own the supported path for power and restart actions, troubleshooting, image update handling, and the required browser-based operator workflow.

### Decision 6: product location

**Decision:** UDS VM should live as Defense Unicorns-authored software in a future `uds-vm` repository outside `uds-core`, and should not start in `uds-packages`.

UDS VM should be managed in a future `uds-vm` repository as its own Defense Unicorns product codebase on top of UDS Core. That product boundary should keep the runtime package, bundle, VM-specific test assets, airgap image declarations, and release tooling together so VM testing, CI, release workflows, and supporting tooling stay in sync.

The current implementation boundary already follows that shape: `uds-core` owns the thin enablement hooks for policy, namespace integration, VM mutation, and capability enablement in the `Package` API, while the VM capability owns the KubeVirt and CDI deployment, VM-specific platform configuration, packaging, tests, and bundle assembly. This capability should ship independently on top of UDS Core rather than as part of the standard Core package because it has its own hardware and runtime prerequisites, support boundary, and release cadence.
...
- UDS Core integration for service exposure, security, policy exceptions, registry secret propagation, and required VM annotations
- Baseline runtime logging, VM health and resource metrics, troubleshooting, and image update handling
...
- **Image delivery default:** For in the first iteration, should UDS VM use the image import methods that already work today, or should it use the new approach that splits large VM images into smaller pieces for Zarf delivery?

## Longer-term questions

These questions do not block the first iteration release, but they shape follow-on design work.

- **Browser console direction:**  Which first supported browser-based management path best fits UDS VM: reuse of an existing OSS KubeVirt management UI, a Defense Unicorns-owned wrapper around that UI, or a more custom implementation?
