...
The bundle author defines *which* Helm values and Zarf variables are configurable and where they map. The deployer provides the *values* via `uds-config.yaml` at deploy time. This separation lets you build the bundle once and configure it specific to each cluster.
