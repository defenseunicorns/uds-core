...
> [!NOTE]
> Zarf Values is currently an **Alpha** feature and may change as it matures (see [zarf-dev/zarf#3946](https://github.com/zarf-dev/zarf/issues/3946)). UDS Core requires UDS CLI `v0.34.0` or later for its values workflow. Packages can still use [Zarf Variables](https://docs.zarf.dev/ref/examples/variables/) for deploy-time inputs.
...
Use `###ZARF_VAR_<NAME>###` in ordinary `valuesFiles` when a variable should be substituted directly. Use `templatedValuesFiles` with `.Variables.<NAME>` when explicit package values must take precedence over variables or need a computed fallback. Keep those phases separate because Zarf substitutes variable tokens before evaluating Go templates.
