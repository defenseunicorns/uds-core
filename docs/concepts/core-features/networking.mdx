...
> [!IMPORTANT]
> The Admin Gateway should be reachable only from trusted networks by privileged users (not standard application end users). Expose the Tenant Gateway as your public (or broadly reachable) ingress and restrict the Admin Gateway to private/internal networking, a VPN, a bastion, or a restricted subnet. This pattern allows end users to still reach applications (including Keycloak for SSO), while ensuring that admin interfaces are never reachable from the public internet. This is typically configurable through load balancer controller specific annotations, see [restrict the admin gateway to internal networks](/getting-started/production/build-your-bundle/#restrict-the-admin-gateway-to-internal-networks) for examples and references on implementing this.
