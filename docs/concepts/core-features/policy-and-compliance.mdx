...
> [!NOTE]
> The full list of enforced policies, including which are mutations vs. validations and any configuration options, is documented in the [UDS Operator Policies](/reference/operator-policies/) reference.

## Exemptions

Some workloads legitimately require behavior that policy would otherwise block — a privileged DaemonSet for node-level observability, or a legacy application that cannot yet run as non-root. UDS Core handles these cases through the `UDSExemption` custom resource.
...
> [!TIP]
> Ready to configure policies for your environment? See the [Policy & Compliance How-to Guides](/how-to-guides/policy-and-compliance/overview/). For a full list of enforced policies, see the [UDS Operator Policies](/reference/operator-policies/) reference.
