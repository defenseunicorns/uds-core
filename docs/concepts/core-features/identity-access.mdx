...
## Identity & Authentication in UDS Core
...
- Restarts pods that consume secrets labeled `uds.dev/pod-reload` when credentials rotate
...
<Tabs>
  ...
  <TabItem label="Authservice (fallback)">
    ...
    **Limitations to be aware of:**
    - Authservice handles authentication at the proxy layer, but the application itself has no awareness of user identity — without a native OIDC flow, the application cannot map requests to specific users or enforce user-level authorization
...
When using private PKI or self-signed certificates, UDS Core provides a trust bundle mechanism that propagates CA certificates to platform components (including Keycloak). This ensures that the SSO flow does not break when operating in air-gapped environments with internally-issued certificates.

---

> [!TIP]
> Ready to configure identity for your environment? See the [Identity & Access How-to Guides](/how-to-guides/identity-access/overview/).
