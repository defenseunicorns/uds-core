...
The operator authenticates to Keycloak using Federated Signed JWTs backed by projected Kubernetes Service Account tokens. This eliminates the need for a shared client secret between the operator and Keycloak. The authentication method is configurable via the `KEYCLOAK_CLIENT_MODE` setting in the `uds-operator-config` Secret.
