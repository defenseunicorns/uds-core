...
> [!NOTE]
> The `unicorn` flavor is only available in a private container registry. It requires a Defense Unicorns support agreement. [Contact Defense Unicorns](https://www.defenseunicorns.com/contactus) for access.
...
> [!TIP]
> **Compare CVE counts:** You can view current CVE counts for the `upstream` and `registry1` flavors on the [UDS Registry Core Package](https://registry.defenseunicorns.com/repo/public/core/versions). The `unicorn` flavor undergoes additional patching and curation by Defense Unicorns, resulting in significantly fewer CVEs — [contact Defense Unicorns](https://www.defenseunicorns.com/contactus) to learn more.
