...
## Authenticating with the unicorn registry

The `unicorn` flavor container images are hosted on `cgr.dev/defenseunicorns.com`. Pulling these images locally requires authentication via `chainctl`, the Chainguard CLI, configured as a Docker credential helper. See [Unicorn registry authentication](/dev/unicorn-registry-auth/) for setup instructions.
