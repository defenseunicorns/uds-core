...
| [core-portal](https://github.com/defenseunicorns/uds-core/tree/main/packages/portal) | UDS Portal (application portal UI; upstream flavor only) | Base, Identity & Authorization |
...
| **Portal** | Include to give users a browser-based portal listing the applications they can access; upstream flavor only |

> [!NOTE]
> The Monitoring layer includes Grafana, which requires the Identity & Authorization layer for login.

> [!NOTE]
> The Portal layer is only available in the `upstream` flavor. It is not included in `registry1` or `unicorn` flavor builds.
...
- `core-monitoring`
- `core-portal` (upstream flavor only)
