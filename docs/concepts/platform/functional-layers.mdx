...
| [core-portal](https://github.com/defenseunicorns/uds-core/tree/main/packages/portal) | UDS Portal (application portal UI; not available in `registry1`) | Base, Identity & Authorization |
...
| **Portal** | Include to give users a browser-based portal listing the applications they can access; not available in `registry1` |

> [!NOTE]
> The Monitoring layer includes Grafana, which requires the Identity & Authorization layer for login.

> [!NOTE]
> The Portal layer is not available in the `registry1` flavor.
