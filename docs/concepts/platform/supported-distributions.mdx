...
> [!NOTE]
> UDS Core tests EKS, AKS, and RKE2 against **Kubernetes 1.35**. The target is typically **n-1** (one minor version behind the latest release, latest patch). This version may lag slightly behind new Kubernetes releases.

| Distribution | K8s Version | Status | Testing Schedule |
|-------------|-------------|--------|-----------------|
| [K3s](https://k3s.io/) / [k3d](https://k3d.io/) | **1.34**, **1.35** | [![Kubernetes Compatibility](https://github.com/defenseunicorns/uds-core/actions/workflows/test-kubernetes-matrix.yaml/badge.svg?branch=main&event=schedule)](https://github.com/defenseunicorns/uds-core/actions/workflows/test-kubernetes-matrix.yaml?query=event%3Aschedule+branch%3Amain) | Nightly and on release-please pull requests |
...
The [Kubernetes compatibility workflow](https://github.com/defenseunicorns/uds-core/blob/main/.github/workflows/test-kubernetes-matrix.yaml) tests the exact K3s releases in the [K3s matrix](https://github.com/defenseunicorns/uds-core/blob/main/.github/kubernetes-test-matrix.yaml). This matrix does not define the UDS Core support policy. Kubernetes release support is tracked by the [Kubernetes release calendar](https://kubernetes.io/releases/).
