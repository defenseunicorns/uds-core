...
- **Networking** — API server CIDR, node CIDRs
- **TLS** — custom CA certificates
- **Domains** — tenant and admin domains
- **External integrations** — database endpoints for Keycloak/Grafana HA, external object storage for Loki/Velero

## What Stays the Same

Across every environment tier you deploy the **same packages at the same version**, the **same policy baseline** (Pepr policies, Istio authorization), and the **same observability stack** (Prometheus, Loki, Grafana). This consistency makes staging-to-production promotion predictable — if it works in staging with production-like config, it will work in production.
