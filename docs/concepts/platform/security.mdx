...
- **Authservice integration** handles the browser OIDC flow and enforces complete protection at the mesh edge, reducing the amount of authentication logic each application must implement.
...
> [!NOTE]
> **Why it matters:** Access control is centralized and auditable. Authservice-protected applications get platform-managed authentication enforcement without implementing the browser OIDC flow themselves. Native SSO applications retain ownership of application login, sessions, and domain-specific authorization.
