...
UDS Core takes a layered approach to security, enforcing controls at every stage from software supply chain through runtime behavior. This page summarizes each security layer and how they work together.
...
- **Admission control** enforced by UDS policies via [Pepr](https://docs.pepr.dev/) (non-root, drop capabilities, block privileged/host access, etc.).
...
- **Compliance-ready**: controls are designed to address requirements in NIST 800-53, DISA STIG, and FedRAMP baselines to support ATO processes.
...
UDS Core provides centralized identity management through Keycloak and Authservice. Authservice is a platform-owned authentication boundary for workloads that use platform-managed OIDC protection:

- **Keycloak SSO** with opinionated defaults for realms, clients, and group-based access control.
- **Authservice integration** handles the browser OIDC flow and enforces complete protection at the mesh edge, reducing the amount of authentication logic each application must implement. `groups.anyOf` can add a platform-level coarse-grained access gate.
- **Consistent Keycloak identity and group mapping** across applications, with platform-managed login and token handling for Authservice-protected workloads.

> [!NOTE]
> **Why it matters:** Access control is centralized and auditable. Authservice-protected applications get platform-managed authentication and coarse-grained group access without implementing the browser OIDC flow themselves, while application code can use forwarded identity and configured group claims for fine-grained domain authorization. Native SSO applications retain ownership of application login, sessions, and domain-specific authorization.

[Identity & Authorization concepts &rarr;](/concepts/core-features/identity-and-authorization/)
...
The security controls documented on this page are designed with regulated environments in mind. UDS Core helps address control families commonly evaluated across NIST 800-53, DISA STIG, and FedRAMP baselines.

If your organization is pursuing an **Authority to Operate (ATO)** or needs compliance documentation for a regulated environment deployment, Defense Unicorns provides technical documentation and control mapping artifacts to support your authorization effort.

[Contact Defense Unicorns &rarr;](https://www.defenseunicorns.com/contact)
