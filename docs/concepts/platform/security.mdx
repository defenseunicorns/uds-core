...
| Control Family | How UDS Core Addresses It |
|---|---|
| **Access Control (AC)** | RBAC, group-based SSO, Authservice enforcement, admin gateway isolation |
| **Audit & Accountability (AU)** | Centralized log collection via Vector/Loki, searchable audit trail for all workloads |
| **Configuration Management (CM)** | Immutable packaging via Zarf, policy enforcement via Pepr, explicit exemption audit trail |
| **Identification & Authentication (IA)** | Keycloak with MFA support, FIPS 140-2 mode support |
| **System & Communications Protection (SC)** | Istio STRICT mTLS, default-deny NetworkPolicy, zero-trust authorization |
| **System & Information Integrity (SI)** | Falco runtime threat detection, per-release CVE scanning, SBOMs |
