...
## Secret Auto-Reload for SSO Clients

When SSO client secrets are updated or rotated, applications using these secrets may need to be restarted to pick up the new values. UDS Core provides an automatic secret reload mechanism that detects changes to secrets and restarts the relevant pods or deployments.

To enable this functionality for SSO client secrets, you can add the `uds.dev/watch: "true"` label to the secret via the `secretLabels` field in your Package CR. When a secret with this label is updated, UDS Core will either:

1. Restart pods matching the selector specified in the `uds.dev/pod-selector` annotation (which can be added via the `secretAnnotations` field), or
...
For more details on configuring secret auto-reload, see the [Secret Auto-Reload documentation](/reference/deployment/secret-auto-reload) or the [Secret Templating documentation](/reference/configuration/single-sign-on/sso-templating#secret-auto-reload).
