...
The group receives `realm-management/query-users` and a group policy that grants `view` and `manage` scopes on the `Users` resource type. Keycloak has no attribute only scope. The permission can update all user profile fields, delete users, and reset passwords when needed. It does not grant role mappings, group membership, clients, realm settings, or impersonation.
