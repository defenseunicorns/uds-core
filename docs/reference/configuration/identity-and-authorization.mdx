...
UDS Core defaults `sso[].fullScopeAllowed` to `true` for backwards compatibility. Set it to `false` whenever your application works with explicit role scope mappings and client scopes, which limits token roles to only those the client requires. Reference the [`Package` CR SSO fields](/reference/operator-and-crds/packages-v1alpha1-cr/#sso) for the field definition.
