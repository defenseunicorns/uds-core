...
> [!CAUTION]
> Assign group membership to a named federated identity when possible. For a local fallback, the script creates a user with a temporary password. Do not store users or passwords in Helm values or a realm import. To revoke access, remove the user from `uds-realm-user-admins` and log out the user's sessions.
