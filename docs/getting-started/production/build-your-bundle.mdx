...
- [AWS Load Balancer Controller](https://kubernetes-sigs.github.io/aws-load-balancer-controller/latest/guide/service/annotations/) - set `service.beta.kubernetes.io/aws-load-balancer-scheme: "internal"`
- [Azure Kubernetes Service](https://learn.microsoft.com/en-us/azure/aks/internal-lb) - set `service.beta.kubernetes.io/azure-load-balancer-internal: "true"`
- [Google Kubernetes Engine](https://cloud.google.com/kubernetes-engine/docs/how-to/internal-load-balancing) - set `networking.gke.io/load-balancer-type: "Internal"`
- [MetalLB](https://metallb.io/usage/#requesting-specific-ips) - set `metallb.io/address-pool: <pool-name>` to pin the gateway's load balancer IP to a dedicated internal address pool (configure the pool itself via `IPAddressPool`)
  ...
  - name: core
    overrides:
      uds-operator-config:
        uds-operator-config:
          values:
            - path: clusterConfig.spec.expose.pathRouting
              value: true
            - path: clusterConfig.spec.expose.contextPath
              value: "/platform"
            - path: clusterConfig.spec.expose.adminContextPath
              value: "/admin"
```
...
2. **Inspect the bundle** _(optional)_
