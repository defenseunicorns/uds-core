...
Configure NTP on every worker node as part of your node image build or bootstrap process. UDS Core components such as Keycloak and Loki are sensitive to host clock drift: skewed node time can cause short-lived SSO sessions, token validation failures, and unstable `loki-write` ring membership.

Verify that your node image enables a time sync service by default, or configure one during bootstrap.

If you run on EKS, you can use the AWS Time Sync Service (`169.254.169.123`) with the time sync daemon used by your node image. Use the equivalent NTP source and daemon configuration for your platform if you are not running on EKS.
...
If you are using **[Cilium](https://cilium.io/)**, CIDR-based network policies require an additional [feature flag](https://docs.cilium.io/en/stable/security/policy/layer3/#selecting-pods-or-nodes-with-cidr-ipblock) set to `nodes` for node addressability.
...
- NTP is configured on all worker nodes
