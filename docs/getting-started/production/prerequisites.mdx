...
On EKS, AWS provides a link-local time source at `169.254.169.123`. If your base image uses `systemd-timesyncd`, add that address under `[Time]` in `/etc/systemd/timesyncd.conf` and restart `systemd-timesyncd`.

Use the NTP source required by your platform if you are not running on EKS.
