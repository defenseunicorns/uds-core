...
3. **Mount the trust bundle in your application**
   ...
   Platform components (Keycloak, Grafana, Loki, etc.) automatically mount the trust bundle; no manual configuration is needed. For your own applications, mount the `uds-trust-bundle` ConfigMap as a volume.
...
- [Configure TLS certificates](/how-to-guides/networking/configure-tls-certificates/) - Set up TLS certificates for your ingress gateways, often paired with trust bundle configuration.
- [Identity & Authorization how-to guides](/how-to-guides/identity-and-authorization/overview/) - Configure SSO with Keycloak, which may need trust bundle configuration for private PKI.
