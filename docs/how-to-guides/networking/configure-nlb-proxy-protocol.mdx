...
2. **Configure the NLB to send PROXY protocol v2**
   ...
   Enable PP2 on the NLB target group that points at the gateway service. The exact step varies by provider; for AWS, set `proxy_protocol_v2.enabled = true` on the target group attribute (Terraform: `aws_lb_target_group.proxy_protocol_v2 = true`). Without this step the gateway will see the NLB's IP rather than the real client IP, but external traffic will still succeed because the filter is permissive.
