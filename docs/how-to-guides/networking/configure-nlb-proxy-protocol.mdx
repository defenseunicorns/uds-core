...
> [!WARNING]
> **Security: source IP spoofing risk.** Once enabled, Envoy trusts a PROXY protocol v2 header from any client that can reach the gateway listener. A workload that can connect directly to the ingressgateway pods (bypassing the upstream load balancer) can forge any source IP by sending its own PP2 header. This affects anything keyed on client IP, such as access logs, IP-based access control, and rate limiting. It does not affect certificate-based authentication (for example, CAC), which is keyed off the TLS client certificate, not the source IP. Before enabling PP2, restrict network access from outside the cluster (security groups, NACLs, VPC firewalls) so only the upstream load balancer can reach the gateway listener.
