...
> [!TIP]
> If your SAN certificate covers both `*.yourdomain.com` and `yourdomain.com`, set `tls.servers.tenant.includeRootDomain: true`. Do not set `rootDomain.enabled` or configure `rootDomain.tls`. Reference [Expose applications on gateways](/how-to-guides/networking/expose-apps-on-gateways/) for the full shared-certificate configuration.
