...
> [!TIP]
> If your SAN certificate covers both `*.yourdomain.com` and `yourdomain.com`, set `rootDomain.enabled: true` and `rootDomain.server: tenant`. Do not configure `rootDomain.tls` or `rootDomain.enableHttpsRedirect`. Reference [Expose applications on gateways](/how-to-guides/networking/expose-apps-on-gateways/) for the full shared-certificate configuration.
