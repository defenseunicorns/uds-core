...
If you are planning to [expose an app on the root (apex) domain](/how-to-guides/networking/expose-apps-on-gateways/), choose a dedicated root certificate or reuse a SAN certificate from the tenant server. The following configuration creates a dedicated root certificate:
...
> [!TIP]
> If your SAN certificate covers both `*.yourdomain.com` and `yourdomain.com`, set `tls.servers.tenant.includeRootDomain: true`. Do not set `rootDomain.enabled` or configure `rootDomain.tls`. Reference [Enable root domain support](/how-to-guides/networking/expose-apps-on-gateways/#optional-enable-root-domain-support) for the full shared-certificate configuration.
