...
4. **(Optional) Configure advanced HTTP routing**
   ...
   **Example: Add response headers and configure retries**
   ...
   UDS Core adds HTTP Strict Transport Security (HSTS) with `includeSubDomains` to Keycloak's public and admin routes. Ensure that any subdomains beneath those Keycloak hostnames also use HTTPS. This is not a gateway-wide policy.
