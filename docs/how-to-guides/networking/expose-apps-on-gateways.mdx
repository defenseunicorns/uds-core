...
1. **(Optional) Enable root domain support**
   ...
   Use this configuration when the tenant server certificate covers both `*.yourdomain.com` and `yourdomain.com`. The shared server controls TLS mode, minimum TLS version, and HTTPS redirects. Do not set `rootDomain.enabled` or configure `rootDomain.tls` fields with this option.
...
**Solution:**
- Confirm either `rootDomain.enabled` or `tls.servers.tenant.includeRootDomain` is set to `true` in your bundle overrides, not both
