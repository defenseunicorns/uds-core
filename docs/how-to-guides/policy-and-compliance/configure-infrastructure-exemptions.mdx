...
Infrastructure `Exemption`s are typically applied during or before Core installation to resolve infrastructure-specific issues that would otherwise block deployment. For application-level `Exemption`s, deploy manifests alongside the applications instead; see [Create UDS policy exemptions](/how-to-guides/policy-and-compliance/create-policy-exemptions/).
