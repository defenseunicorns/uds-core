...
3. **Configure Prometheus storage and retention**
   ...
   > [!CAUTION]
   > If stored data exceeds PVC capacity, Prometheus will crash-loop. Always provision PVC size with headroom above your expected retention volume. `retentionSize` acts as a safety cap — Prometheus drops the oldest blocks when this limit is reached. To resize PVCs on a running cluster, see the [Resize Prometheus PVCs](/operations/troubleshooting-and-runbooks/resize-prometheus-pvc/) runbook.
...
**Solution:** Check PVC usage and Prometheus logs:
```bash
