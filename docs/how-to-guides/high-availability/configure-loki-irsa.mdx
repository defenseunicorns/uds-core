...
- Loki configured with external S3 storage (see [Configure HA for Logging](/how-to-guides/high-availability/logging/))

## Before you begin

> [!NOTE]
> This guide covers IRSA authentication for Loki running in production with external S3 storage. If you have not yet connected Loki to S3, complete the [Configure HA for Logging](/how-to-guides/high-availability/logging/) guide first and return here to replace static credentials with IRSA.
...
1. **Create an IAM policy for Loki S3 access**
   ...
   Create a policy that grants Loki read, write, and delete access to both its S3 buckets. Loki's compactor requires `s3:DeleteObject` to expire old chunks according to the configured retention period.
