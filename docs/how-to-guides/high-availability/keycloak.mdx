...
2. **Enable HPA autoscaling**
   ...
   | Minimum available replicas | Not set | `podDisruptionBudget.minAvailable` |
   | Unhealthy pod eviction policy | `AlwaysAllow` | `podDisruptionBudget.unhealthyPodEvictionPolicy` |

   If you set `podDisruptionBudget.minAvailable`, the chart omits the default `maxUnavailable` because Kubernetes does not allow both fields.

   Set `podDisruptionBudget.enabled` to `true` for a fixed-replica deployment. This explicit setting also preserves a fixed-replica PDB whose configuration matches the new chart defaults, including a partial override that sets only default values.
