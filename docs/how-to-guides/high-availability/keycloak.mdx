...
2. **Enable HPA autoscaling**
   ...
   Enabling `autoscaling.enabled` creates a PodDisruptionBudget (PDB) by default. Set `podDisruptionBudget.enabled` to `false` to disable it in HA mode. Outside HA mode, the chart never creates a PDB, regardless of this flag or other PDB settings.

   Before upgrading a fixed-replica deployment with a chart-managed PDB, configure HA with an external PostgreSQL database and enable autoscaling if you need to retain chart-managed PDB protection.

   The default PDB configuration protects the remaining Ready replica during voluntary disruptions:

   | Setting | Default | Override path |
   |---|---|---|
   | Enabled in HA mode | `true` | `podDisruptionBudget.enabled` |
   ...
   The chart defines these defaults in `values.yaml`. To use `podDisruptionBudget.minAvailable`, set `podDisruptionBudget.maxUnavailable` to `null` to remove its default. The chart rejects configurations that set both fields, including when the PDB is disabled.
