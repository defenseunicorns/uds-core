...
**Solution:** Check host clock sync on the nodes running `keycloak`. Keycloak token and session validation is sensitive to clock skew. If the affected nodes disagree on time, fix NTP on the hosts first, then restart `keycloak` only if the issue persists.
