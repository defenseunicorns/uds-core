...
2. **Enable HPA autoscaling**
   ...
   The chart sets `podDisruptionBudget.maxUnavailable` to `1`. Do not set `podDisruptionBudget.minAvailable`; Helm fails when both fields are present because Kubernetes does not allow both fields.
