...
The following example shows how to define `values` and map them into chart values within the Zarf package definition:
...
The following example shows a `values.yaml` with defaults a deployer can override:
...
The following example shows a `zarf-values.schema.json` that validates the defaults above:
