...
5. **Define the UDS `Package` CR**
   ...
   **SSO**: register a Keycloak client if your app has a user login. Choose native OIDC or SAML when the application should own the authentication flow. Choose [Authservice](/how-to-guides/identity-and-authorization/protect-apps-with-authservice/) when UDS Core should own browser authentication and enforce protection at the mesh boundary.
