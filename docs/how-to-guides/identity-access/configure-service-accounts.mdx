...
You'll configure a Keycloak client using the [OAuth 2.0 Client Credentials Grant](https://oauth.net/2/grant-types/client-credentials/) so that automated processes — CI/CD pipelines, backend services, and scripts — can obtain tokens and access Authservice-protected applications without a user session.

## Prerequisites

- UDS Core deployed
- A UDS Package CR for the workload that needs machine-to-machine access
- The `clientId` of the Authservice-protected application you want to access (used as the token audience)
...
- The `aud` (audience) claim is **not** set by default — you must add an audience mapper to allow the token to access a specific Authservice-protected application
...
3. **Retrieve the client secret**
   ...
   The client secret is stored in a Kubernetes secret named after the SSO client:

   ```bash
   uds zarf tools kubectl get secret -n <namespace> <client-id> -o jsonpath='{.data.secret}' | base64 -d
   ```
...
A successful response includes an `access_token`. Decode it at [jwt.io](https://jwt.io) or with `jq` to verify the `aud` claim includes the expected audience.
...
Check the decoded token's `aud` claim:
```bash
# Decode the access token (replace TOKEN with the actual token value)
...
- [Package CR reference](/reference/operator-and-crds/package/) — full SSO client and `protocolMappers` field specification
- [Protect apps with Authservice](/how-to-guides/identity-access/protect-apps-with-authservice/) — how to set up the Authservice-protected application that service accounts will access
...
<CardGrid>
  <LinkCard
    ...
    description="Enable device authorization for CLI tools and headless apps."
