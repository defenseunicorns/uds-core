...
This flow creates a **public client** — a client with no secret. Two important constraints follow from this:

- `standardFlowEnabled` must be set to `false` (public clients cannot use the standard redirect flow)
...
1. **Configure the Package CR for device flow**
   ...
   > [!NOTE]
   > No Kubernetes secret is created for public clients — there is no client secret to store. Your application initiates device flow by calling the Keycloak device authorization endpoint directly.
...
2. **Apply the Package CR**
   ...
   The UDS Operator creates the Keycloak client in the UDS realm.
...
```bash
# Initiate device authorization (replace with your domain and client ID)
curl -s -X POST \
...
### Problem: Users can complete device flow but cannot access Authservice-protected resources

**Symptoms:** Token obtained via device flow is rejected by Authservice-protected applications.

**Solution:** Authservice validates tokens against a specific client. A device flow token issued to a public client will not have the correct `aud` claim for an Authservice-protected application unless you configure an audience mapper. See [Configure service account clients](/how-to-guides/identity-access/configure-service-accounts/) for an example of adding audience mappers — the same approach applies here.
...
- [Package CR reference](/reference/operator-and-crds/package/) — full SSO client field specification
- [Identity & Authentication concepts](/concepts/core-features/identity-access/) — background on the UDS SSO model
