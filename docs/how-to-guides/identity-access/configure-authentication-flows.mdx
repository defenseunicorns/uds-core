...
> [!NOTE]
> When upgrading UDS Core, realm configuration (`realmAuthFlows` and `realmInitEnv`) does **not** propagate automatically to a running Keycloak instance — a full redeploy of the Keycloak package is required to apply changes. Theme files, truststore certificates, and custom plugin JARs **do** apply automatically on upgrade without a realm redeploy.
...
4. **(Optional) Enable X.509/CAC with MFA**
   ...
   If you need to require MFA after CAC authentication, use this configuration in the `realmAuthFlows` block from step 2 in place of the values shown there, then run `uds deploy uds-bundle.yaml`:
...
- [Configure Keycloak password policy and account settings](/how-to-guides/identity-access/configure-user-account-settings/) — set password complexity and email verification alongside auth flow configuration
