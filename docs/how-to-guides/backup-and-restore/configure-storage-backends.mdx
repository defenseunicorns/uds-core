...
description: Configure Velero's backup storage destination using S3-compatible storage or Azure Blob Storage, with options for static credentials, existing Kubernetes Secrets, or IRSA on AWS EKS.
sidebar:
  order: 7.001
---
   ...
   <Tabs>
     <TabItem label="S3-compatible storage">
       ...
       <Tabs>
         ...
         <TabItem label="IRSA on AWS EKS">

           IRSA (IAM Roles for Service Accounts) lets Velero assume an IAM role without storing credentials in the cluster. The EKS OIDC webhook injects temporary credentials that the AWS SDK uses automatically.

           For full configuration steps, including IAM policy and role setup, see [Configure Velero with IRSA on AWS EKS](/how-to-guides/backup-and-restore/configure-velero-irsa/).
...
- [Configure Velero with IRSA on AWS EKS](/how-to-guides/backup-and-restore/configure-velero-irsa/) - authenticate Velero to S3 using IAM Roles for Service Accounts instead of static access keys.
