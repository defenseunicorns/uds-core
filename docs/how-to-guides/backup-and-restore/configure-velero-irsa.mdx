...
description: Configure Velero to authenticate to S3 using IAM Roles for Service Accounts (IRSA) on AWS EKS, replacing static access keys with temporary credentials.
sidebar:
  order: 7.002
---
...
- [OpenTofu](https://opentofu.org/) or Terraform installed locally to provision IAM resources
...
IRSA works by annotating the Velero service account (`velero/velero-server`) with an IAM role ARN (Amazon Resource Name). The IRSA webhook automatically injects temporary credentials into the Velero pod, which the AWS SDK uses in place of a credentials file.
...
2. **Create an IAM role with an IRSA trust policy**
   ...
   Place both `.tf` files in the same directory. Supply `oidc_provider` and `aws_account_id` via `-var` flags or a `terraform.tfvars` file, then apply:
