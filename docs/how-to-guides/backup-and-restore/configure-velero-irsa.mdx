...
1. **Create an IAM policy for S3 access**
   ...
   Create a policy with the permissions Velero requires to read, write, and manage backup objects:
...
3. **Configure your bundle for IRSA**
   ...
   > [!TIP]
   > To also capture persistent volume data using EBS snapshots, see [Enable volume snapshots (AWS EBS)](/how-to-guides/backup-and-restore/enable-volume-snapshots-aws-ebs/). That guide assumes IRSA is already configured on the Velero service account.
