...
2. **Specify which hosts display the banner**
   ...
   > [!TIP]
   > Host values support Helm templating. Use `{{ .Values.domain }}` to reference your configured domain rather than hardcoding hostnames.
...
### Banner not appearing on a host
...
```bash
uds zarf tools kubectl get envoyfilter -n istio-system -o yaml | grep -A 20 classificationBanner
```

### Banner breaks page layout or doesn't render correctly
...
<CardGrid>
  <LinkCard
    ...
    description="Another cross-cutting platform feature: restart pods automatically when Secrets or ConfigMaps change."
