...
2. **Deploy the rules**
   ...
   Apply the ConfigMap to your cluster. The Loki sidecar detects the `loki_rule: "1"` label and loads the rules automatically.
   ...
   > [!NOTE]
   > The sidecar watches for ConfigMap changes continuously. Updates to existing ConfigMaps are picked up without any manual reload.

3. **Optional: Create recording rules**
   ...
   ```bash
   uds zarf tools kubectl apply -f loki-recording-rules.yaml
   ```

4. **Optional: Alert on recorded metrics**
   ...
   ```bash
   uds zarf tools kubectl apply -f prometheus-rule-from-logs.yaml
   ```

   > [!TIP]
   > For more details on PrometheusRule CRs, see [Create metric alerting rules](/how-to-guides/monitoring-observability/create-metric-alerting-rules/).
...
<CardGrid>
  <LinkCard
    ...
    description="Configure Alertmanager to deliver Loki alerts to Slack, PagerDuty, or email."
    href="/how-to-guides/monitoring-observability/route-alerts-to-notification-channels/"
  />
  <LinkCard
    title="Create metric alerting rules"
    description="Define additional alerting conditions based on Prometheus metrics."
