   ...
   spec:
     ...
     - name: my-app
       ...
       - alert: HighMemoryUsage
         expr: |
           (container_memory_working_set_bytes / kube_pod_container_resource_limits{resource="memory"}) * 100 > 80
         for: 15m
         labels:
           severity: warning
         annotations:
...
2. **Deploy the rule**
   ...
   Apply the PrometheusRule CR to your cluster. The Prometheus Operator picks it up automatically — no bundle rebuild is needed.
...
<CardGrid>
  <LinkCard
    ...
    description="Configure Alertmanager to deliver your alerts to Slack, PagerDuty, or email."
  ...
  <LinkCard
    ...
    description="Complement metric alerts with log pattern detection using Loki Ruler."
