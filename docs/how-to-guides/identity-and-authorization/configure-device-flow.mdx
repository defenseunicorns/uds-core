...
This flow creates a **public client** (a client with no secret). Two important constraints apply to device-flow public clients in UDS Core:

- `standardFlowEnabled` must be explicitly set to `false`. The UDS operator will reject the `Package` CR if it is not. Device-flow-only public clients are admitted by default; other public-client shapes require the cluster admin to enable `ALLOW_PUBLIC_CLIENTS` (see [Configure user accounts and security policies](/how-to-guides/identity-and-authorization/configure-user-account-settings/)).
