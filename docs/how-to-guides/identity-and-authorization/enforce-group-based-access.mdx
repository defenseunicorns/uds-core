...
2. Access should be denied. Depending on the integration, denial can occur during Keycloak authentication or as a `403 Forbidden` response from the application.

**For Authservice-protected applications, check the generated policy:**
```bash
uds zarf tools kubectl get authorizationpolicy -n <app-namespace>
```

For native SSO applications, verify the group requirement on the Keycloak client and test the application's login flow. Native SSO alone does not create an Authservice chain.
