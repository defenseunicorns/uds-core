...
2. **Add `groups.anyOf` to your `Package` CR**
   ...
   In your application's `Package` CR, add a `groups.anyOf` list under the relevant SSO client. Users must be a member of at least one of the listed groups to be granted access. Choose the example that matches the SSO integration you use.

   <Tabs>
     <TabItem label="Native SSO">

     The application owns the login flow, so omit `enableAuthserviceSelector`:
     ...
     Authservice owns the browser login flow and protects the selected workload at the mesh boundary:
