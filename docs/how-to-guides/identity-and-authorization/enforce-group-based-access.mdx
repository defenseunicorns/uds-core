...
The `groups` token claim can help debug what an application receives, but it does not control `groups.anyOf` enforcement. To inspect token claims, use the Keycloak Account console at `sso.<domain>` or a tool like [jwt.io](https://jwt.io).
