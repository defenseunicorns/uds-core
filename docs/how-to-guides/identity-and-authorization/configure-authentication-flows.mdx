...
### Problem: Username/password login still appears after it is disabled

**Symptoms:** The login page still displays username/password login even though `USERNAME_PASSWORD_AUTH_ENABLED: false` was set in the bundle override.
...
If redeploying is not possible, configure the username/password-related flows manually in the Keycloak Admin Console at `keycloak.<admin_domain>` → **uds** realm:

| Flow setting | Admin UI path |
|---|---|
| Disable username/password | **Authentication** → **Flows** → **UDS Authentication** → expand **Username Password Form** and set **Deny Access** to **Required** |
...
If you also need OTP or WebAuthn, configure it separately:

| Optional flow setting | Admin UI path |
|---|---|
