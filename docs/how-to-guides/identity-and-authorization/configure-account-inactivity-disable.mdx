...
1. **Set `accountInactivityDays` in your bundle override**
   ...
   > [!NOTE]
   > This setting is applied during initial realm import. If Keycloak is already running, the workflow state is not updated by changing this value alone. Use the Keycloak Admin Console to enable or update the workflow on a running instance.
...
6. Return to the Admin Console, open the workflow, and click **View active instances** — a new instance should appear for the test user with a `disable-user` step scheduled at 35 days
...
**Solution:** The `accountInactivityDays` value was `0` at deploy time or was not applied. Verify the bundle override was included and the value is greater than `0`. To enable on a running instance without redeploying, toggle **Enabled** directly in the Admin Console and set the **after** duration on the `disable-user` step.
