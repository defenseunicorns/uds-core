...
This guide covers the bundle override approach. For CSS or structural theme changes, see [Build and deploy a custom identity config image](/how-to-guides/identity-and-authorization/build-deploy-custom-image/).
...
6. **(Optional) Disable registration form fields**
   ...
   > [!NOTE]
   > Unlike `realmInitEnv`, `themeCustomizations.settings` values are applied at runtime. Keycloak does not need to be redeployed for them to take effect.
     ...
     keycloak:
       keycloak:
         values:
           - path: themeCustomizations.settings
             value:
               realmDisplayName: "Unified Defense Stack"
   ```

   > [!NOTE]
   > If `realmDisplayName` is not set, the login page falls back to the realm's own display name, which may be set at initial realm import via `realmInitEnv.DISPLAY_NAME`.
...
> [!NOTE]
> `realmInitEnv` values are applied only during initial realm import. If Keycloak is already running, Keycloak must be fully torn down and redeployed for these values to take effect.
...
- [uds-identity-config repository](https://github.com/defenseunicorns/uds-identity-config) - source repository with theme assets and FreeMarker templates for deeper customization
...
- [Build a custom Keycloak configuration image](/how-to-guides/identity-and-authorization/build-deploy-custom-image/) - Build and deploy a custom image for CSS or structural theme changes.
