...
> [!TIP]
> **This guide is for applications where the application should own the OIDC or SAML flow.** If UDS Core should own browser authentication and enforce protection at the mesh boundary, see [Protect applications with Authservice](/how-to-guides/identity-and-authorization/protect-apps-with-authservice/).
...
The application reads its credentials from this secret and speaks directly to Keycloak. The application handles the SSO flow instead of delegating browser authentication to Authservice.
...
3. **(Optional) Add protocol mappers for custom token claims**
   ...
   > [!NOTE]
   > Custom protocol mappers and client scopes are subject to Keycloak's security hardening policy. If Keycloak rejects your mapper or scope, add it to the allow list via `SECURITY_HARDENING_ADDITIONAL_PROTOCOL_MAPPERS` or `SECURITY_HARDENING_ADDITIONAL_CLIENT_SCOPES`. Non-device-flow public clients are gated separately at the UDS Operator admission layer via `ALLOW_PUBLIC_CLIENTS` in the `uds-operator-config` Secret. See [Public clients](/reference/configuration/identity-and-authorization/#public-clients).
...
4. **(Optional) Configure client attributes**
   ...
   | `pkce.code.challenge.method` | Require PKCE. Public clients must set this to `S256` (RFC 7636, case-sensitive); `plain` is rejected by admission. |
