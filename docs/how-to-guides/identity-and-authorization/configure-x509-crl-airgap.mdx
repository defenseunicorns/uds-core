...
- X.509/CAC authentication enabled in UDS Core — see [Configure Keycloak authentication methods](/how-to-guides/identity-access/configure-authentication-flows/) and [Configure the CA truststore](/how-to-guides/identity-access/configure-truststore/)
...
> [!NOTE]
> ImageVolumes are currently blocked by UDS policies. You will need to add a policy exemption as part of this guide.
...
1. **Build the CRL Zarf package**
   ...
   **Use a pre-downloaded ZIP (recommended for airgap transfer):**
   ...
   Download the ZIP on a connected machine, transfer it into the airgap or build system, then run the script locally to produce the package.
...
4. **Add a policy exemption for ImageVolumes**

   UDS policies currently block `ImageVolume` volume types. Add an exemption targeting Keycloak pods:
...
5. **(Optional) Enable ImageVolume on k3d for Kubernetes < 1.35**

   If you are running on `uds-k3d` with Kubernetes < 1.35, enable the `ImageVolume` feature gate via `uds-config.yaml`:
...
In the Keycloak admin console at `keycloak.<admin_domain>` → **uds** realm → **Authentication** → **Policies** → **X.509 authenticator settings**, verify the CRL Distribution Points value matches the contents of `keycloak-crl-paths.txt`.
...
**Solution:** The UDS policy exemption was not applied or did not match the pod. Verify:
- The exemption is included in the deployed bundle
- The namespace is `keycloak` and the pod matcher `^keycloak-.*` matches the Keycloak pod name
...
<CardGrid>
  <LinkCard
    ...
    description="Enable or disable X.509/CAC, OCSP, and CRL revocation checking via bundle overrides."
