...
2. **Verify the CR exists and check its status**
   ...
   **What to look for:** the `status.phase` should be `Ready`. If it's stuck on `Pending` or shows an error, the operator is not successfully reconciling it — see [Cause 2: Incorrect CR definition](#cause-2-incorrect-cr-definition).
   ...
   > [!NOTE]
   > `Exemption` CRs must be created in the `uds-policy-exemptions` namespace unless you have configured a [namespace override](/reference/operator-and-crds/exemptions-v1alpha1-cr/).
...
- [Exemptions specification](/reference/operator-and-crds/exemptions-v1alpha1-cr/) — CR schema, field reference, and namespace override options
