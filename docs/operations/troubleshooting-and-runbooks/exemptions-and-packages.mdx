...
1. **Check pepr-system pod health**
   ...
   ```bash title="Check controller pods"
...
2. **Verify the CR exists and check its status**
   ...
   ```bash title="Check Package CR status"
   uds zarf tools kubectl get packages -n <namespace> <package-name> -o jsonpath='{.status}'
   ```
   ...
   **What to look for:** the `status.phase` should be `Ready`. If it's stuck on `Pending` or shows an error, the operator is not successfully reconciling it.
   ...
   ```bash title="Check Exemption CR"
   uds zarf tools kubectl get exemptions -n uds-policy-exemptions <exemption-name>
   ```

   > [!NOTE]
   > `Exemption` CRs must be created in the `uds-policy-exemptions` namespace unless you have configured a [namespace override](/reference/operator-and-crds/exemption/).

3. **Check exemption processing logs**

   ```bash title="Grep for exemption processing"
   uds zarf tools kubectl logs -n pepr-system deploy/pepr-uds-core | grep "Processing exemption"
   ```

   **Look for:** log entries similar to:

   ```json title="Expected exemption log entry"
...
1. **Check pod events for failure reasons**
   ...
   ```bash title="Describe failing pods"
...
2. **Restart the controller deployments**

   ```bash title="Restart all pepr-system deployments"
   uds zarf tools kubectl rollout restart deploy -n pepr-system
   ```

3. **Verify pods recover**

   ```bash title="Watch pod recovery"
...
2. **Collect metrics from the watcher:**
   ...
   ```bash title="Collect watcher metrics"
...
- [Packages specification](/reference/operator-and-crds/package/) — CR schema and field reference
- [Exemptions specification](/reference/operator-and-crds/exemption/) — CR schema, field reference, and namespace override options
