...
- **Password reset links:** Administrator-triggered password reset emails for locked-out users now contain usable `sso.<domain>` links. Self-service resets already used this public hostname. Realm administration remains on `keycloak.<admin_domain>`, and the paired hostname configuration hardens URL generation by keeping public and administrative URLs on their configured origins ([#2898](https://github.com/defenseunicorns/uds-core/pull/2898)).
