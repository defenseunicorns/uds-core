...
> [!IMPORTANT]
> Upgrade to v1.3.1 rather than v1.3.0. A misconfigured Prometheus Operator value in v1.3.0 causes kubelet metrics to not be scraped after node rotation due to `EndpointSlices` not being updated ([#2647](https://github.com/defenseunicorns/uds-core/pull/2647)). If you cannot upgrade immediately, set `prometheusOperator.kubeletEndpointsEnabled: true` in the kube-prometheus-stack values via a bundle override.
