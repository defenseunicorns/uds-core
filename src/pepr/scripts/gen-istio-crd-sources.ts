 ...
 * Or via the task: uds run -f src/pepr/tasks.yaml gen-crds
 */
...
async function main() {
  ...
  // Grab the VirtualService ${CRD_VERSION} OpenAPI schema
  const vsVersion = vs.spec?.versions?.find(v => v?.name === CRD_VERSION);
  const vsSchema = vsVersion?.schema?.openAPIV3Schema;
  if (!vsSchema) throw new Error(`VirtualService ${CRD_VERSION} schema missing`);
  ...
  const outPath = path.resolve(
    __dirname,
    `../operator/crd/sources/istio/virtualservice-${CRD_VERSION}.ts`,
  );
  const header = [
    "/**",
    ` * Auto-generated from Istio ${ISTIO_VERSION} CRD (virtualservices.networking.istio.io ${CRD_VERSION}).`,
    " * Generated by src/pepr/scripts/gen-istio-crd-sources.ts",
    " * Usage: uds run -f src/pepr/tasks.yaml gen-crds",
