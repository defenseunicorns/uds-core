...
describe("enableIstio with kubevirt", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    (K8s as Mock).mockImplementation(resourceKind => {
      if (resourceKind === kind.Pod) {
  ...
  test("sets kubevirt workload label and annotation when kubevirt.enabled is true", async () => {
    mockGet.mockResolvedValue({ metadata: { labels: {}, annotations: {} } });
    const pkg: UDSPackage = {
      metadata: { namespace: "test-ns", name: "test-pkg" },
  ...
  test("does not set kubevirt label when kubevirt.enabled is false", async () => {
    mockGet.mockResolvedValue({ metadata: { labels: {}, annotations: {} } });
    const pkg: UDSPackage = {
      metadata: { namespace: "test-ns", name: "test-pkg" },
      spec: { kubevirt: { enabled: false } },
    };

    await enableIstio(pkg);

    const appliedMeta = mockApply.mock.calls[0][0].metadata;
    expect(appliedMeta.labels["uds.dev/kubevirt-workload"]).toBeUndefined();
    expect(appliedMeta.annotations["uds.dev/kubevirt-pkg-test-pkg"]).toBeUndefined();
  });

  test("sets kubevirt label when any of multiple packages has kubevirt enabled", async () => {
    mockGet.mockResolvedValue({
      metadata: {
        labels: {},
        annotations: {
          "uds.dev/kubevirt-pkg-pkg-a": "true",
        },
      },
    });
    const pkg: UDSPackage = {
      metadata: { namespace: "test-ns", name: "test-pkg-b" },
      spec: { kubevirt: { enabled: false } },
    };

    await enableIstio(pkg);

    // Label should remain because pkg-a still has kubevirt enabled
    expect(mockApply).toHaveBeenCalledWith(
      expect.objectContaining({
        metadata: expect.objectContaining({
          labels: expect.objectContaining({ "uds.dev/kubevirt-workload": "true" }),
...
describe("cleanupNamespace with kubevirt", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    (K8s as Mock).mockImplementation(resourceKind => {
      if (resourceKind === kind.Pod) {
  ...
  test("removes kubevirt annotation and clears label when last kubevirt package is removed", async () => {
    mockGet.mockResolvedValue({
      metadata: {
        labels: { "istio.io/dataplane-mode": "ambient", "uds.dev/kubevirt-workload": "true" },
        annotations: {
          "uds.dev/pkg-test-pkg": "true",
          "uds.dev/kubevirt-pkg-test-pkg": "true",
          "uds.dev/original-istio-state": IstioState.None,
    ...
    // Original istio state was None, so all istio labels are removed
    // Kubevirt label is cleared because no kubevirt annotations remain
    expect(mockApply).toHaveBeenCalledWith(
      expect.objectContaining({
        metadata: expect.objectContaining({
          labels: {},
  ...
  test("keeps kubevirt label when other kubevirt packages remain", async () => {
    mockGet.mockResolvedValue({
      metadata: {
        labels: { "istio.io/dataplane-mode": "ambient", "uds.dev/kubevirt-workload": "true" },
        annotations: {
    ...
    // Other package still present, so labels kept as-is; kubevirt label remains
    expect(mockApply).toHaveBeenCalledWith(
      expect.objectContaining({
        metadata: expect.objectContaining({
          labels: expect.objectContaining({ "uds.dev/kubevirt-workload": "true" }),
...
describe("ensurePrivateRegistrySecret via enableIstio", () => {
  const mockSecretGet = vi.fn();
  const mockSecretCreate = vi.fn().mockResolvedValue({});
  ...
  beforeEach(() => {
    ...
    (K8s as Mock).mockImplementation(resourceKind => {
      ...
      if (resourceKind === kind.Secret) {
        return { Get: mockSecretGet, Create: mockSecretCreate };
      }
      if (resourceKind === UDSPackage) {
        return { InNamespace: vi.fn().mockReturnValue({ Get: mockPackageGet }) };
      }
  ...
  test("skips copy when private-registry secret already exists in target namespace", async () => {
    mockGet.mockResolvedValue({ metadata: { labels: {}, annotations: {} } });
    mockSecretGet.mockResolvedValue({ metadata: { name: "private-registry", namespace: "test-ns" } });
    const pkg: UDSPackage = {
      metadata: { namespace: "test-ns", name: "test-pkg" },
      spec: { kubevirt: { enabled: true } },
    };
  ...
  test("copies secret from source namespace when missing from target", async () => {
    mockGet.mockResolvedValue({ metadata: { labels: {}, annotations: {} } });
    // First call: target namespace - not found
    // Second call: source namespace (pepr-system) - found
    mockSecretGet
      .mockRejectedValueOnce(new Error("not found"))
      .mockResolvedValueOnce({
        metadata: { name: "private-registry", namespace: "pepr-system" },
    ...
    expect(mockSecretCreate).toHaveBeenCalledWith({
      metadata: { name: "private-registry", namespace: "test-ns" },
      type: "kubernetes.io/dockerconfigjson",
      data: { ".dockerconfigjson": "base64data" },
    });
  });

  test("does not call secret API when kubevirt is not enabled", async () => {
    mockGet.mockResolvedValue({ metadata: { labels: {}, annotations: {} } });
    const pkg: UDSPackage = {
      metadata: { namespace: "test-ns", name: "test-pkg" },
      spec: { kubevirt: { enabled: false } },
  ...
  test("warns and skips when no source secret is found anywhere", async () => {
    mockGet.mockResolvedValue({ metadata: { labels: {}, annotations: {} } });
    // All secret Get calls fail
  ...
  test("falls back to global scan when candidate namespaces have no secret", async () => {
    mockGet.mockResolvedValue({ metadata: { labels: {}, annotations: {} } });
    // Target: not found. Candidates: not found. Global scan: found.
    mockSecretGet
      .mockRejectedValueOnce(new Error("not found")) // target
      .mockRejectedValueOnce(new Error("not found")) // pepr-system
      .mockRejectedValueOnce(new Error("not found")) // zarf
      .mockRejectedValueOnce(new Error("not found")) // uds-crds
      .mockResolvedValueOnce({
        // global scan
        items: [
          {
            metadata: { name: "private-registry", namespace: "some-system-ns" },
            type: "kubernetes.io/dockerconfigjson",
            data: { ".dockerconfigjson": "globaldata" },
          },
        ],
      });
    const pkg: UDSPackage = {
...
describe("nsEntryIsOverClaimed with kubevirt annotations", () => {
  test("returns false for kubevirt-pkg annotations", () => {
    expect(
      nsEntryIsOverClaimed({
        fieldsV1: {
          "f:metadata": {
            "f:annotations": {
              "f:uds.dev/kubevirt-pkg-test-pkg": {},
            },
          },
        },
      }),
  ...
  test("returns true for unrecognized annotations", () => {
    expect(
      nsEntryIsOverClaimed({
        fieldsV1: {
          "f:metadata": {
            "f:annotations": {
              "f:some-random-annotation": {},
            },
          },
        },
      }),
