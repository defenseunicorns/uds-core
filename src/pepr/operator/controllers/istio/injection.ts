...
export async function enableInjection(pkg: UDSPackage) {
  ...
  if (!annotations[pkgKey] || originalInjectionLabel !== "enabled") {
    ...
    log.debug(
      `Updating namespace ${pkg.metadata.namespace} with istio injection label`,
    );
...
export async function cleanupNamespace(pkg: UDSPackage) {
  ...
  // Apply the updated Namespace
  log.debug(
    `Updating namespace ${pkg.metadata.namespace}, removing istio injection labels.`,
  );
  await K8s(kind.Namespace).Apply(
    {
      metadata: {
...
async function killPods(ns: string, enableInjection: boolean) {
  ...
  for (const pod of pods.items) {
    ...
    if (pod.metadata?.deletionTimestamp) {
      log.debug(
        `Ignoring Pod ${ns}/${pod.metadata?.name}, already being deleted`,
      );
      continue;
    }

    const foundSidecar = pod.spec?.containers?.find(
      c => c.name === "istio-proxy",
    );

    // If enabling injection, ignore pods that already have the istio sidecar
    if (enableInjection && foundSidecar) {
      log.debug(
        `Ignoring Pod ${ns}/${pod.metadata?.name}, already has sidecar`,
      );
      continue;
    }
    ...
    const controlledBy =
      pod.metadata?.ownerReferences?.find(ref => ref.controller)?.uid ||
      "other";
    groups[controlledBy] = groups[controlledBy] || [];
    log.debug(
      `Adding Pod ${ns}/${pod.metadata?.name} to ${controlledBy} deletion list.`,
    );
  ...
  for (const group of Object.values(groups)) {
    ...
    for (const pod of group) {
      log.info(
        `Deleting pod ${ns}/${pod.metadata?.name} to enable the istio sidecar`,
      );
