...
import { advancedHTTP } from "../istio/virtualservice-v1";
...
const expose = {
  type: "array",
  description:
    "Expose a service through UDS Core managed ingress. HTTP entries use Istio; UDP entries use Envoy Gateway.",
  items: {
    type: "object",
    anyOf: [
      {
    ...
    properties: {
      ...
      gateway: {
        description:
          "The name of the gateway to expose the service on. HTTP defaults to tenant when omitted. UDP uses the UDS Core managed default Gateway when omitted. For UDP user-managed Gateways, this value is used as both the Gateway name and namespace, and that namespace must be deployed via a Zarf package (for the private registry image pull secret) and enrolled in the cluster's service mesh (for the mTLS-protected connection to the Envoy Gateway controller); otherwise the managed proxy will fail to start.",
        type: "string",
      },
      domain: {
        description:
...
const sso = {
  ...
  items: {
    ...
    properties: {
      ...
      fullScopeAllowed: {
        description:
          "Controls which roles Keycloak includes in tokens for this client. When enabled, Keycloak includes all roles assigned to the authenticated subject. When disabled, Keycloak limits roles according to the client's role scope mappings and linked client scopes. UDS Core defaults this setting to true for backwards compatibility. Set this to false whenever possible to limit role scope and harden security.",
        type: "boolean",
      },
      standardFlowEnabled: {
        description:
