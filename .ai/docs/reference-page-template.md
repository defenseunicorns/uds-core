...
Reference pages live under `docs/reference/<section>/` and use the `.mdx` extension. **Exception:** autogenerated CRD reference pages (`docs/reference/operator-and-crds/*-cr.md`) use `.md` — they are produced by `src/pepr/docs-gen/main.ts` and contain raw HTML tables with no MDX components; MDX's stricter HTML parser causes errors with content from CRD descriptions.
